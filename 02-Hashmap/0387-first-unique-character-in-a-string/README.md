...
### 2026-09-11 (1회차) — 통과 · **공간복잡도 오판 교정** · `1일` → `3일` 단계

```
고정 12/12  ·  랜덤 30만건 불일치 0건
```

코드는 09-08과 동일. **달라진 건 복잡도 답변이다 — `O(n)` → `O(1)`.**

#### ⭐ `#공간복잡도오판` 교정 — 3일 연속 정답

```
[측정] Map 최대 엔트리 수 (소문자 26가지)
  n=   100  →  Map 13개
  n= 10000  →  Map 26개
  n=100000  →  Map 26개      ← n이 10배 늘어도 26에서 멈춤
```

**`Map` 이든 `Array` 든 키가 소문자 26가지면 `O(1)`.**

| | `\|Σ\|` | `n` 최대 | 승자 | 답변 |
|---|---|---|---|---|
| [0217. Contains Duplicate](../0217-contains-duplicate/README.md) (09-10) | 20억 | 10만 | `n` | `O(n)` ✅ |
| [0349. Intersection of Two Arrays](../0349-intersection-of-two-arrays/README.md) (09-10) | 1001 | 1000 | `n` | `O(n)` ✅ |
| **0387 (09-11)** | **26** | **10만** | **알파벳** | **`O(1)`** ✅ |

**앞의 둘은 `n` 이 이겼고 이번엔 알파벳이 이겼는데 셋 다 구분했다.**
09-08에는 같은 코드로 `O(n)` 이라 답했던 자리다. → [시간·공간 복잡도](../../concepts/complexity.md)
...
> ### 🔑 `O(1)` 이라는 판단과 "배열로 바꾼다"는 결정은 **같은 사실에서 나온다**
> 복잡도를 정확히 세면 최적화가 따라온다.
> 이번에 `O(1)` 을 맞혔으니 **다음엔 코드도 배열로 나오면 완성**이다.

**판정**: 정답 · 공간복잡도 오판 교정 · 피드백 0회 → `1일` → **`3일` 단계** (다음 09-16)
