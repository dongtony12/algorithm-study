...
### 2026-09-10 (2회차) — 정답 · **09-08 오답 미재발** / 시간복잡도 힌트 1회 · `1일` 단계 유지

```
고정 15/15  ·  랜덤 40만건(길이 자유) 불일치 0건
```
...
**09-08의 `letterNum > 0` 오답 미재발** ✅ — `!== 0` 으로 양쪽을 다 검사한다.
**공간 `O(1)` 정확** (26칸 고정 배열).

---

#### ⭐ ②와 ③을 구분해냄 — 전날 0392 에서 리셋이 났던 자리

1차 답변은 `O(n)`(정의 없음), 힌트 1회 후 **`O(m + n)`**.

```
[0392. Is Subsequence](../../03-Two-Pointers/0392-is-subsequence/README.md):  while 하나에 i, j 가 함께 전진   →  ③  →  O(n)      더하지 않는다
0242              :  for(s) 끝나고 → for(t) 가 시작   →  ②  →  O(m + n)  더한다
```

**`s` 를 100자로 고정하고 `t` 만 늘린 측정:**

```
s=100, t=  100  →   3ms
s=100, t= 1000  →  13ms
s=100, t=10000  → 140ms
s=100, t=50000  → 722ms      ← s 는 그대로인데 240배
```

> ### 🔑 루프가 "나란히"면 더하고, "안에"면 곱하고, "한 루프 안에서 함께 전진"이면 상한 큰 쪽 하나
> 어제 두 문제에서 헷갈렸던 것을 오늘 **같은 `O(m+n)` 을 한쪽에선 쓰고 한쪽에선 안 쓰는** 판단으로 구분했다.

⚠️ **`1 <= s.length, t.length <= 5*10^4` — 두 입력이 독립적으로 5만까지.**
`s.length == t.length` 는 **보장되지 않는다.** 이게 09-08에 오답이 났던 바로 그 이유.

---

#### 길이 가드는 여전히 없다

`!== 0` 이 길이 차이를 **간접적으로** 잡아준다 — 길이가 다르면 어떤 칸이든 `0` 이 아니게 되므로. **그래서 정답이다.**

다만 비용이 다르다:

```
길이 불일치 입력 (s=100, t=50000) × 5000회
  가드 없음 (제출본): 745ms
  가드 먼저        :   0ms
```
...
```ts
function isAnagram(s: string, t: string): boolean {
    if (s.length !== t.length) return false      // ← 한 줄

    const counts = new Array<number>(26).fill(0)
    for (let i = 0; i < s.length; i++) {         // ← 길이가 같으니 한 루프로
        counts[s.charCodeAt(i) - 97]++
        counts[t.charCodeAt(i) - 97]--
    }
    for (const n of counts) if (n !== 0) return false
...
>
> ### 🔑 가드 한 줄이 **축을 하나로 접는다**
> `m = n` 이 보장되므로 복잡도 표기가 `O(m+n)` 이 아니라 **`O(n)`** 이 된다.
> 가드는 성능만이 아니라 **복잡도 서술까지 단순하게** 만든다.

**판정**: 정답 · 09-08 오답 미재발 · 공간복잡도 정확 · ②③ 구분 성공 / 시간복잡도 힌트 1회 → `1일` **유지** (다음 09-11)
