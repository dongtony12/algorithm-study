...
### 2026-08-19 (2회차) — 통과 · **복잡도는 잡혔고, `#루프상한혼동` 은 또 재발**

고정 13건 + 랜덤 30만건(빈 문자열 포함) 불일치 0. 최대 입력(200×200) 2000회 103ms.
...
08-03·08-12의 `slice(0, i)` 대신 **`prefix` 에 글자를 누적**하는 변형. 둘 다 `O(m × k)` / `O(1)`.

#### ✅ `#복잡도차원뭉개기` 드디어 잡힘 (이 문제에서 3번째 만에)
...
기호도 스스로 정의함 — *"i 최대 = 가장 짧은 문자열의 길이, j = strs의 길이"*.

다만 1차 답변에서 **안쪽 루프가 `length - i` 만큼 줄어든다**고 잘못 셌다. 격자 그림으로 교정:
**`i`(열)를 하나 옮길 때마다 그 열의 문자열을 전부 확인**해야 하므로 안쪽은 **항상 `m`번**이고 줄어들지 않는다.

#### ⚠️ `#루프상한혼동` 재발 — 2회

**접근 단계에서 정확히 정의한 직후 구현에서 뒤집혔다.**

```
접근:  i = 열(글자 위치) → k번   /   j = 행(문자열) → m번        ✅
구현:  i < strs.length           /   j < minJLength             ❌ 뒤바뀜
       let target = strs[i][0]                                   ❌ 인덱스 순서도 뒤바뀜
```

08-12에는 같은 실수로 `strs[3]` → `undefined` → **TypeError** 까지 났었다.
이번엔 코드 제출 전 *"이런식으로 접근하는거 맞나"* 로 물어봐서 실행 전에 잡혔다.

> ### 🔑 머리로는 아는데 **손이 뒤집는다** — 이해가 아니라 **표기 습관**의 문제
> ```ts
> for (let i = 0; i < minJLength; i++) {        // i = 글자 위치 (열)
>     for (let j = 0; j < strs.length; j++) {   // j = 문자열 번호 (행)
> ```
> **주석 두 줄이 유일한 대책이다.** 그리고 `strs[?][?]` 는 **첫 대괄호가 문자열 번호, 두 번째가 글자 위치**.
>
> **다음 복습 때 이 주석이 있는지부터 확인한다.**

#### 미재발 ✅

`#공간복잡도오판`(08-03) · `#변수명불명확`(한글 변수명 `기준`, 08-03) 둘 다 미재발.

**판정**: `#루프상한혼동` 재발 → `1일` 단계 유지 (다음 복습 08-20)

### 2026-08-26 (3회차) — 통과, **피드백 0회** · 🎉 `#루프상한혼동` 클리어

고정 13건 + 랜덤 30만건(빈 문자열 포함) 불일치 0. 최대 입력(200×200) 2000회 106ms.
복잡도 `O(m × n)` / `O(1)` 정확 (`m` = strs.length, `n` = minJLength).
...
#### 🎉 `#루프상한혼동` 클리어 — 대책이 실제로 작동했다

| | 08-03 | 08-12 복습 | 08-19 복습 | **08-26 복습** |
|---|---|---|---|---|
| 루프 상한 뒤바꿈 `#루프상한혼동` | — | ❌ (`TypeError`) | ❌ 재발 | ✅ **미재발** |
| 축을 `n` 하나로 뭉갬 `#복잡도차원뭉개기` | ❌ | ❌ | ✅ | ✅ |
| 공간 `O(n)` 오판 `#공간복잡도오판` | ❌ | ✅ | ✅ | ✅ |

**08-19에 남긴 대책 — "루프 옆에 역할을 주석으로 적는다" — 을 실제로 실행했고, 그 결과 상한이 정확히 맞았다.**

> 제안했던 것과 `i`/`j` 이름이 반대지만 **상관없다.**
> 중요한 건 **본인이 정한 역할과 상한이 일치하는 것**이고, 그게 맞았다.

**⚠️ 다만 주석 첫 줄이 오기다** — *"i는 글자들의 개수"* → **"문자열의 개수"**.
다음 줄이 바로잡아주긴 하지만, **주석이 대책인 만큼 주석 자체가 정확해야 한다.**

#### 복잡도 기호 정의

```
m = strs.length            문자열 개수
n = minJLength             가장 짧은 문자열 길이 (= 비교할 열의 개수)
```
