...
### 🔴 2단계 진행 (2026-09-10 변경)

```
【1단계】 전 카테고리 Easy 완주        ← 지금 여기
          주제 순서대로 쭉 민다. 졸업 판정 없이 진도만 나간다.

【2단계】 현재 방식으로 복귀
          주제별 졸업 판정(대표 2개 백지 2연속) + Medium 라운드
```

**왜 바꿨나**: 졸업 판정이 **진도를 막는 게이트**로 작동하고 있었다.
그리고 **복습(백지 재작성)이 이미 같은 일을 하고 있어** 중복이다 — 투 포인터 5문제 중 4개가 이미 복습 대기열에서 백지로 돌고 있다.

> **1단계에서는 주제를 넘어갈 때 졸업 판정을 하지 않는다.** 목록을 다 풀면 다음 주제로 간다.
> 재현 능력 확인은 **복습 대기열이 담당**한다 (`1→3→7→21→60` 확장 간격).

### 주제 졸업 기준 — **2단계에서 적용**
> **그 주제 대표 문제 2개를 연속으로 백지에서 — 힌트 없이 + 복잡도까지 정확히 — 통과하면 졸업.**

문제 수가 아니라 **재현 능력**이 기준.
*(1단계에서는 보류. `배열·문자열`·`해시맵` 은 이미 이 기준으로 졸업했다)*
...
⚠️ **예외 — 그래프 · 백트래킹 · 힙**: Easy가 거의 없어(백트래킹은 사실상 전무) 입문 Medium 2~3개를 1단계에 포함.

| 주제 | 1단계에 포함하는 Medium | 이유 |
|---|---|---|
| 11. 그래프 | `200. Number of Islands` | Top150 Easy 0개, 그래프 입문 필수 |
| 12. 백트래킹 | `78. Subsets` · `46. Permutations` · `77. Combinations` | Easy가 사실상 전무 |
| 13. 힙 | `215. Kth Largest Element in an Array` | 힙의 대표 용례 |

**그 외 주제에서는 1단계에 Medium을 꺼내지 않는다.**
...
| 3 | 투 포인터 | `03-Two-Pointers` | 5 / 5 | ✅ **완료** (졸업 판정은 2단계에서) |
| 4 | **슬라이딩 윈도우** | `04-Sliding-Window` | **0 / 4** | 🔵 **진행 중** |
